...
export const useDebouncedClick = (delay: number = 200) => {
  ...
  const debouncedClick = useCallback((action: () => void, actionName: string) => {
    const now = Date.now();
    if (now - lastClickTime > delay) {
      ...
      console.log(`⏱️ ${actionName} button click ignored (debounced) - ${now - lastClickTime}ms since last click`);
    }
  }, [lastClickTime, delay]);
